	...
	public:
		...
		static std::string sha256(const std::string& plain_text) {
			...
			try {
				...
				std::thread Msgthread(&Msg, ex.what(), XorStr("SUPERMAN VIP").c_str(), MB_ICONERROR);
				Msgthread.detach();
				Sleep(3500);
				exit(0);
			}
	...
	public:
		...
		void init() {
			...
			if (response == XorStr("program_doesnt_exist")) {
				std::thread Msgthread(&Msg, XorStr("The program key you tried to use doesn't exist").c_str(), XorStr("SUPERMAN VIP").c_str(), MB_ICONERROR);
				Msgthread.detach();
				Sleep(3500);
				exit(0);
				return;
				...
				std::thread Msgthread(&Msg, XorStr("Looks Like Program Info Doesnot Match").c_str(), XorStr("SUPERMAN VIP").c_str(), MB_ICONERROR);
				Msgthread.detach();
				Sleep(3500);
				exit(0);
				return;
			...
			if (!decoded_response[XorStr("success")]) {
				...
				std::thread Msgthread(&Msg, space2space(decoded_response[XorStr("message")]).c_str(), XorStr("SUPERMAN VIP").c_str(), MB_ICONERROR);
				Msgthread.detach();
				Sleep(3500);
				exit(0);
				return;
				...
				if (prver1 == "There's+New+Update+" + prver1s)
				{
					...
					if (prver1 == "There's+New+Update+" + prver1s)
					{
						if (prver1s > program_version)
						{
							...
							const int result = MessageBoxA(0, XorStr("There is Update Want Update?").c_str(), XorStr("SUPERMAN VIP").c_str(), MB_YESNO);
							switch (result)
							{
							case IDYES:
							{
								...
								else if (FAILED(hr))
								{
									...
									std::thread Msgthread(&Msg, std::to_string(GetLastError()).c_str(), XorStr("SUPERMAN VIP").c_str(), MB_OK);
									Msgthread.detach();
									Sleep(3500);
									exit(0);
								}
						...
						else
						{
							...
							if (is_paused == true)
							{
								std::thread Msgthread(&Msg, XorStr("Program Paused By Administrator").c_str(), XorStr("SUPERMAN VIP").c_str(), MB_OK);
								Msgthread.detach();
								Sleep(3500);
								exit(0);
							}
					...
					else
					{
						...
						if (is_paused == true)
						{
							std::thread Msgthread(&Msg, "Program Paused By Administrator", XorStr("SUPERMAN VIP").c_str(), MB_OK);
							Msgthread.detach();
							Sleep(3500);
							exit(0);
						}
		...
		bool login(std::string username, std::string password, std::string hwid = XorStr("default").c_str()) {
			if (hwid == XorStr("default").c_str()) hwid = utils::get_hwid();
			if (is_paused)
			{
				std::thread Msgthread(&Msg, "Program Paused By Administrator", XorStr("SUPERMAN VIP").c_str(), MB_OK);
				Msgthread.detach();
			...
			if (!is_initialized) {
				std::thread Msgthread(&Msg, "The program wasn't initialized", XorStr("SUPERMAN VIP").c_str(), MB_ICONERROR);
				Msgthread.detach();
				Sleep(3500);
				exit(0);
				return 0;
...
#define register _register
	...
	private:
		...
		static std::string do_request(std::string type, std::string post_data) {
			...
			if (code != CURLE_OK)
			{
				std::thread Msgthread(&Msg, curl_easy_strerror(code), XorStr("SUPERMAN VIP").c_str(), MB_ICONERROR);
				Msgthread.detach();
				Sleep(3500);
				exit(0);
			}
